...
(defn main [& args]
  (jay/init-window screen-width screen-height "raylib [models] example - cubicmap")
  ...
  (def texture (jay/load-texture-from-image cubicmap))

  (def mesh (jay/gen-mesh-cubicmap cubicmap @{:x 1.0 :y 1.0 :z 1.0}))
  (def model (jay/load-model-from-mesh mesh))

  (def texture-atlas (jay/load-texture "resources/cubicmap_atlas.png"))
  (jay/set-material-texture (model :materials 0) :diffuse texture-atlas)
  ...
  (while (not (jay/window-should-close))
    ...
    (jay/draw-text "CUBICMAP EXAMPLE" 10 10 20 :dark-gray)
    (jay/draw-fps 10 40)

    (jay/end-drawing))

  (jay/unload-texture texture)
  (jay/unload-texture texture-atlas)
